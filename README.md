...
> **English TL;DR** — Arduino library that plays 16-bit PCM WAV files from the SD card through a MAX98357 I2S amplifier on HUB8735 Ultra / RTL8735B (AmebaPro2) boards. It talks directly to the low-level mbed `i2s_api` (already compiled into the AmebaPro2 Arduino package), since the official SDK ships no I2S playback library for this board. Wiring: BCLK→D24, LRC→D12, DIN→D11. See the pin table below.
